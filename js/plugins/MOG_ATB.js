...
Game_BattlerBase.prototype.executeStatesEffect_ATB = function(stateId) {
	...
	this.states().forEach(function(state) {
		if (state.id === stateId) {
			// 自動解除タイミングが「なし」以外だったら解除フラグを立てる
			// 今のところ「行動終了時」「ターン終了時」は厳密に考えない
			// see: https://github.com/nagaiwork/spear_and_maid_01/issues/3
			isRemove = state.autoRemovalTiming !== 0;

			// HP,MP,TP再生率の特徴が設定されている場合だけリジェネ処理を行う
			// 再生率の特徴があるステートが同時に複数掛かっている場合、再生率の効果が重複するバグは発生するが、
			// 今のところ再生率の特徴があるのは「毒」だけなので、一旦この修正にする
			// see: https://github.com/nagaiwork/spear_and_maid_01/issues/2
			if (state.traits.length !== 0) {
				state.traits.forEach(function(trait){
					// code: 22=追加能力値
					// dataId: 7=HP再生率, 8=MP再生率, 9=TP再生率
					if (!isRegenerate) {
						isRegenerate = trait.code === 22 && [7, 8, 9].includes(trait.dataId);
					}
				});
			}
		}
	...
	if (this._stateTurns[stateId] === 0 && isRemove) {this.eraseState(stateId)};
};
...
BattleManager.executeATBFullEffect = function(battler) {
	...
	// 反撃拡張のプラグインが読み込まれていれば、カウンターステート解除のメソッド実行
	if (Imported.CounterExtend) {
		battler.removeCounterState();
	}
};
...
// * カウンターステート解除
//==============================
Game_BattlerBase.prototype.removeCounterState = function() {
	this.states().forEach(function(state) {
		if (state.note.indexOf("<counterState>") != -1) {
			this.eraseState(state.id);
		}
	}, this);
};
...
BattleManager.endAction = function() {
	...
	if (this._subject) { 
		// 反撃拡張のプラグインがある場合の追加処理
		if (Imported.CounterExtend) {
			// 反撃でのアクションではATBを初期化しない
			// ※反撃時は_atbItem(アクション内容)がnullになる
			if (this._subject._atbItem != null) {
